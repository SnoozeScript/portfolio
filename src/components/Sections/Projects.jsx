...
const Projects = () => {
    const getProjectImage = (index) => {
        switch (index) {
            case 0: return farmgeniusPreview;
            case 1: return chatassistantPreview;
            case 2: return otakuhavenPreview;
            case 3: return text2voxPreview;
            default: return farmgeniusPreview;
        }
    };
                    ...
                    >
                        ...
                        <div className="w-full lg:w-3/5 relative perspective-1000">
                            <motion.div
                                ...
                                className="relative rounded-2xl overflow-hidden shadow-2xl border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-gray-900 aspect-video flex flex-col"
                            >
                                {/* Browser Bar */}
                            ...
                            {/* Decorative Elements */}
                            <div className={`absolute -z-10 w-full h-full top-4 ${index % 2 === 0 ? 'left-4' : 'right-4'} rounded-2xl border-2 border-gray-200 dark:border-gray-800 hidden lg:block transition-transform duration-500 group-hover:translate-x-2 group-hover:translate-y-2`}></div>
                        </div>
                        ...
                        <div className="w-full lg:w-2/5 flex flex-col gap-6">
                            <div className="flex flex-col gap-2">
                                ...
                                <h3 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300">
                                    {project.name}
                                </h3>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 md:hidden">
                                {project.tech.map((tech, i) => (
                                    <span key={i} className="px-3 py-1 text-sm bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-full">
                                        {tech}
                                    </span>
                                ))}
                            </div>
                                ...
                                <ul className="space-y-2">
                                    {project.highlights.slice(0, 3).map((highlight, i) => (
                                        <li key={i} className="flex items-start gap-3 text-gray-600 dark:text-gray-400">
                                            <ArrowUpRight size={18} className="text-blue-500 flex-shrink-0 mt-0.5" />
                                            <span>{highlight}</span>
                                        </li>
                                    ))}
                                </ul>
                            ...
                            <div className="flex items-center gap-4 mt-2">
                                ...
                                {project.github && (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="p-3 rounded-full border border-gray-200 dark:border-gray-700 hover:border-gray-900 dark:hover:border-white text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-all hover:scale-110"
